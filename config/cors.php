...
return [
    ...
    */
    ...
    // Кэш preflight-ответа (OPTIONS) в браузере/WebView. `0` = не кэшировать:
    // тогда WebView отправляет preflight **перед каждым** запросом, а синк делает
    // 15 запросов за проход — то есть лишние 15 OPTIONS на каждый sync() (и они тоже
    // упираются в `throttle:api`). Сутки кэша снимают половину нагрузки и убирают
    // «мигающие» 429 у активных устройств (дефект живого прогона 14.11).
    'max_age' => 86400,
