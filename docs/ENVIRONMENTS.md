...
Для **чистых установок** дополнительно выпущен APK `versionCode 16` (`1.15`) уже с новым адресом —
он опубликован на домашний контур, поэтому промо-страница нового контура отдаёт сборку, которая
сразу ходит на новый API. APK-часть не требуется для уже установленных приложений: им достаточно бандла.
